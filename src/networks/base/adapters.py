...
# --------------------------
# The following is for  Capters
# --------------------------

class Capters(nn.Module):
    def __init__(self, in_channels, dim_capsules):
        super(Capters, self).__init__()
        self.caps = nn.ModuleList([nn.Linear(in_channels, dim_capsules) for _ in range(24)])

    def forward(self, x):
        x_caps = [self.squash(capsule(x), axis=-1) for capsule in self.caps]
        x_caps = torch.cat(x_caps, dim=-1)
        return x + x_caps

    def squash(self, x, axis):
        s_squared_norm = (x ** 2).sum(axis, keepdim=True)
        scale = torch.sqrt(s_squared_norm)/ (1 + s_squared_norm)
        return scale * x

class Capters2(nn.Module):
    def __init__(self, dim_capsules):
        super(Capters2, self).__init__()
        self.caps = nn.Linear(int(768/dim_capsules), 768)
        self.dim =  dim_capsules

    def forward(self, x):
        x_squashed = self.squash(x.view(x.size(0), x.size(1), -1, self.dim), axis=-1)
        x_squashed = torch.sqrt((x_squashed ** 2).sum(-1)).squeeze(-1)
        x_caps = self.caps(x_squashed)
        return x + x_caps
...
class CapsNet(nn.Module):
    def __init__(self,config):
        ...
        # self.capters = Capters(in_channels=768, dim_capsules=int(768/24))
        self.capters = Capters2(dim_capsules=16)
        print('CapsNet')

    def forward(self, t, x,s):
        semantic_output = self.semantic_capsules(t,x,s,'semantic')
        output = self.tsv_capsules(t,semantic_output,s,'tsv')
        output = self.capters(x)
        return output
